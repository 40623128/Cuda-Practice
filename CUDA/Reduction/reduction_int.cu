...
//kernel1
__global__ void kernel1(int* arr, int* out, int N){
...
//kernel2
__global__ void kernel2(int* arr, int* out, int N){
    __shared__ int s_data[threadsPerBlock];
    unsigned int tid = threadIdx.x;
    unsigned int i = threadIdx.x + blockIdx.x * blockDim.x;
    if(i < N){
...
//kernel3
__global__ void kernel3(int* arr, int* out, int N){
    __shared__ int s_data[threadsPerBlock];
    unsigned int tid = threadIdx.x;
    unsigned int i = threadIdx.x + blockIdx.x * blockDim.x;
    if(i < N){
...
//kernel4
__global__ void kernel4(int* arr, int* out, int N){
    __shared__ int s_data[threadsPerBlock];
    unsigned int tid = threadIdx.x;
...
//kernel5
__device__ void warpRecude(volatile int* s_data, int tid){ // volatile ??字很重要，保?s_data?相?的?存?元取出，?里??指gpu?存
...
__global__ void kernel5(int* arr, int* out, int N){
    __shared__ int s_data[threadsPerBlock];
    unsigned int tid = threadIdx.x;
    unsigned int i = threadIdx.x + blockIdx.x * (blockDim.x * 2); // 3的第一?迭代，有一半的?程是idle的，?在把一?block的大小?小一半
    if(i < N){
...
__device__ void warpRecude06(volatile int* s_data, int tid){ // volatile ??字很重要，保?s_data?相?的?存?元取出，?里??指gpu?存
...
template<unsigned int blockSize>
__global__ void reduce06(int* arr, int* out, int N){
    __shared__ int s_data[threadsPerBlock];
    unsigned int tid = threadIdx.x;
    unsigned int i = threadIdx.x + blockIdx.x * (blockDim.x * 2); // 3的第一?迭代，有一半的?程是idle的，?在把一?block的大小?小一半
    if(i < N){
...
void kernel6(int* arr, int* out, int N, cudaStream_t &stream){   // 展?所有的循?，去除循?
...
//kernel7
template<unsigned int blockSize>
__device__ void warpRecude07(volatile int* s_data, int tid){ // volatile 很重要，保證s_data從相應的內從單元取出，這裡指gpu內存
...
template<unsigned int blockSize>
__global__ void reduce07(int* arr, int* out, int N){
    __shared__ int s_data[threadsPerBlock];
    unsigned int tid = threadIdx.x;
...
void kernel7(int* arr, int* out, int N, cudaStream_t &stream){   // 展開所有循環，去除循環
    switch(threadsPerBlock){
        case 1024:
...
int main(){
    ...
    for(int k=0; k<kernal_number; k++){
        for(int j=0; j<times_of_average; j++){
            ...
            //記憶體設定(異步)
            cudaMemcpyAsync(a_device, a_host, N * sizeof(int), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(r_device, r_host, blocksPerGrid * sizeof(int), cudaMemcpyHostToDevice, stream);
            ...
            //主機與設備間記憶體複製
            cudaMemcpy(r_host, r_device, blocksPerGrid * sizeof(int), cudaMemcpyDeviceToHost);
